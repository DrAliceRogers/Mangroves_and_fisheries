...
run_model<-function(params, initial.run = FALSE, fish.herbs = TRUE, add.nursery = FALSE, pred.stock.recruitment = FALSE){
...
##*************
#Not sure if I'm going to need this for the invert spectrum yet...
    ...
    for (i in 1:(N-1)) {
      ...
      #************************      
      #Originally added this after the upwind calculations but then it increased with time steps - i.e. they built up
      #Adding here - before the time calculation seems to solve the problem! Hurrah! Hopefully all those added are also 
      #now subjected to growth and mortality where they were not before...
      
      if(add.nursery == T){
      
      
      U[ref.x1psup:ref.xmaxpsup, i] <- U[ref.x1psup:ref.xmaxpsup, i] + 
       (supp_preds[ref.x1psup:ref.xmaxpsup] * dt) 
      
      H[ref.x1hsup:ref.xmaxhsup, i] <- H[ref.x1hsup:ref.xmaxhsup, i] + 
       (supp_herbs[ref.x1hsup:ref.xmaxhsup] * dt) 
...
##New feeding rate functions
      ...
      #Feeding on detritus by herbivores ***NEW***
...
# NOTE FROM JLB ALTERNATIVE DETRITUS: 
...
# # this one assumes immeidate additional losses of all detritus to sediment?
# # dW<-input.w - (output.w + W[i])     
...
# IS THIS ONE RELEVANT FOR ALGAE?:
...
if (pred.stock.recruitment==T) {
      ...
      U[ref,i+1]<-U[ref,i] + 
        (sum(R.u[(ref+1):end,i]*10^x[(ref+1):end]*U[(ref+1):end,i]*dx)*dt)/(dx*10^x[ref]) - (dt/dx)*(1/log(10))*(GG.u[ref,i])*U[ref,i]-dt*Z.u[ref,i]*U[ref,i]
...
#-------------------------
    ...
    FP_prod_gr<-colSums(10^x[Fref:Fref2]*GG.u[Fref:Fref2,]*U[Fref:Fref2,]*dx)
    ...
    #Proportional refuges
      ...
      return(list(Body.size = x, Preds=U[,N-1], P_grth=GG.u[, N-1], P_mrt=PM.u[, N-1],
                   ...
                   Pred_gm_fishable = Pred_bio_fishable[length(Pred_bio_fishable)], Herb_gm_fishable = Herb_bio_fishable[length(Herb_bio_fishable)], 
...
plotsizespectrum<-function(modeloutput,params,timeaveraged=F, add.points=F) {
  ...
  with(params, {  
    ...
    points(log10(modeloutput$Invs)[ref.det:end]~modeloutput$Body.size[ref.det:end], type = "l", col = "brown")
...
plotbiomass <- function(modeloutput, params){
  ...
  with(params, {
    ...
    plot(modeloutput$p_bio_time, type = "l", col = "red", ylim = c(0,max(modeloutput$p_bio_time+10)))
